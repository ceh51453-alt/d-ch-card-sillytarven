...
function extractFeatures(text) {
    ...
    if (pnMatches) {
        for (const n of pnMatches)
            properNouns.add(n);
    }
    return { cjkBigrams, words, placeholders, tags, properNouns };
}
function getFeaturesFor(field) {
...
function ensureDFMap(allFields) {
    // Rebuild if field count changed (new card loaded or fields added)
    if (dfMap && dfDocCount === allFields.length)
        return;
    dfMap = new Map();
...
function tfidfCosineSimilarity(aFeatures, bFeatures) {
    // Merge CJK bigrams and words into unified term vectors
    const aTerms = new Map();
    const bTerms = new Map();
...
function calculateSimilarity(currentFeatures, candidateFeatures) {
    let score = 0;
    let weightSum = 0;
 ...
 */
function getAdaptiveBudget(field, userMaxChars, userMaxFields) {
...
function smartGlossaryMatch(currentText, glossary) {
 ...
 */
export function buildUnifiedRAGContext(input) {
 ...
 */
export function buildUnifiedRAGContextWithDebug(input) {
    const startTime = performance.now();
    const { currentField, allFields, glossary, mvuDictionary, customSchema, translationMemory, } = input;
    // Adaptive budget
    ...
    // Debug collectors
    const debugSelectedFields = [];
    const debugGlossaryHits = [];
    ...
    if (customSchema && customSchema.trim()) {
        ...
        if (schemaWords)
            schemaWords.forEach(w => coveredTerms.add(w.toLowerCase()));
    }
    // ══════════════════════════════════════════════════════════
    ...
    const crossFieldResult = buildCrossFieldSection(currentField, allFields, coveredTerms, maxFields, remainingBudget);
    if (crossFieldResult) {
    ...
    if (translationMemory && translationMemory.length > 0) {
        ...
        for (const tm of translationMemory.slice(0, 5)) {
            ...
            if (tmChars + line.length > tmBudget)
                break;
            tmSection += line;
            tmChars += line.length;
        }
 ...
 */
function buildCrossFieldSection(currentField, allFields, coveredTerms, maxFields, maxChars) {
    const translatedFields = allFields.filter((f) => f.path !== currentField.path && f.status === 'done' && f.translated && f.translated.trim());
    ...
    if (currentEntryIdx) {
        const sameEntryFields = translatedFields.filter(f => {
            if (f.group !== 'lorebook' && f.group !== 'lorebook_keys')
                return false;
    ...
    if (isCodeField) {
        ...
        const schemaFields = translatedFields.filter(f => f.group === 'tavern_helper' &&
        ...
        const logicFields = translatedFields.filter(f => (f.entryType === 'initvar' || f.entryType === 'controller' || f.entryType === 'mvu_logic') &&
            !tier1.some(t => t.field.path === f.path) &&
    ...
    for (const candidate of translatedFields) {
        if (tier1and2Paths.has(candidate.path))
            continue;
        const candidateFeatures = getFeaturesFor(candidate);
        let score = calculateSimilarity(currentFeatures, candidateFeatures);
        // Group bonuses
        if (candidate.group === currentField.group)
            score += 0.12;
        if (candidate.group === 'system')
            score += 0.08;
        // Cross lorebook-keys ↔ lorebook bonus
        if ((currentField.group === 'lorebook_keys' && candidate.group === 'lorebook') ||
    ...
    const addEntry = (sf, maxOrigLen, maxTransLen) => {
 ...
 */
export function buildRAGContext(currentField, allFields, config) {

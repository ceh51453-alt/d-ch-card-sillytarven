 ...
 */
function getFieldTypeExtraPrompt(f) {
    if (isRegexContentField(f))
        return REGEX_EXTRA_PROMPT;
    if (isTavernHelperField(f))
        return TAVERN_HELPER_EXTRA_PROMPT;
    if (f.entryType === 'initvar')
        return INITVAR_EXTRA_PROMPT;
    if (f.entryType === 'mvu_logic' || f.entryType === 'controller')
        return MVU_LOGIC_EXTRA_PROMPT;
    return '';
}
/**
 ...
 */
function getBatchFieldTypeExtraPrompts(batchFields, enableMvuSync) {
    ...
    if (enableMvuSync) {
        if (hasInitvar)
            result += INITVAR_EXTRA_PROMPT;
        if (hasMvuLogic)
            result += MVU_LOGIC_EXTRA_PROMPT;
        if (hasRegex)
 ...
 */
function buildMvuDictInjection(mvuDictionary, isLogic, entryType, translatedSchemaContent) {
 ...
 */
function needsTranslatedSchemaContext(field, batchFields) {
    const check = (f) => f.group === 'regex' ||
        f.group === 'tavern_helper' ||
        f.entryType === 'initvar' ||
        f.entryType === 'controller' ||
        f.entryType === 'mvu_logic';
 ...
 */
function buildTranslatedSchemaBlock(translatedSchemaContent) {
 ...
 */
export function buildEffectivePrompt(options) {
    const { translationPrompt, enableJailbreak, enableObjectiveMode, enableMvuSync, enableRAGContext, field, allFields, batchFields, mvuDictionary, glossary, customSchema, liveSchemaContext, ragMaxFields = 5, ragMaxChars = 3000, entryNameDictionary, regexTriggerDictionary, strictCodePreservation = false, enableModMode = false, modInstructions = '', expertMode = false, translatedSchemaContent, } = options;
...
${modInstructionsBlock}`;
        ...
        if (options.enableEjsSync) {
            modPrompt += buildEjsPromptBlock(options.ejsEntryNameDict || {}, options.ejsKeywordDict || {}, options.ejsDecoratorPreserve ?? true);
        }
...
${glossaryList}`;
        ...
        let schemaForApi = effectiveSchema;
        let glossaryForApi = glossary;
...
${modInstructionsBlock}`;
    ...
    if (options.enableEjsSync) {
        prompt = (prompt || '') + buildEjsPromptBlock(options.ejsEntryNameDict || {}, options.ejsKeywordDict || {}, options.ejsDecoratorPreserve ?? true);
    }
    return {

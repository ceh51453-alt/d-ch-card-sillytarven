 ...
 */
function parseJsonFromAi(responseText) {
 ...
 */
export function detectEjsCard(card) {
    const result = {
        isEjs: false,
        confidence: 0,
        ejsBlockCount: 0,
    ...
    if (Array.isArray(data.extensions?.regex_scripts)) {
        data.extensions.regex_scripts.forEach((rs, i) => {
    ...
    for (const { text, source } of allTexts) {
        ...
        // Detect API usage
        if (getwiRegex.test(text)) {
            result.hasGetwi = true;
            getwiRegex.lastIndex = 0;
        }
        if (activewiRegex.test(text)) {
            result.hasActivewi = true;
            activewiRegex.lastIndex = 0;
        }
        if (getWorldInfoDataRegex.test(text)) {
            result.hasGetwi = true;
            getWorldInfoDataRegex.lastIndex = 0;
        }
        if (defineRegex.test(text)) {
            result.hasDefine = true;
            defineRegex.lastIndex = 0;
        }
        if (getChatMessagesRegex.test(text)) {
            result.hasGetChatMessages = true;
            getChatMessagesRegex.lastIndex = 0;
        }
        if (executeRegex.test(text)) {
            result.hasExecute = true;
            executeRegex.lastIndex = 0;
        }
        if (decoratorRegex.test(text)) {
            result.hasDecorators = true;
        }
    }
    // Calculate confidence
    let score = 0;
    if (result.ejsBlockCount > 0) {
        score += 30;
        result.reasons.push(`${result.ejsBlockCount} EJS blocks`);
    }
    if (result.ejsBlockCount > 10) {
        score += 15;
    }
    if (result.ejsBlockCount > 50) {
        score += 10;
    }
    if (result.entryWithEjsCount > 0) {
        score += 10;
        result.reasons.push(`${result.entryWithEjsCount} entries with EJS`);
    }
    if (result.hasGetwi) {
        score += 15;
        result.reasons.push('getwi() calls');
    }
    if (result.hasActivewi) {
        score += 15;
        result.reasons.push('activewi() dynamic entry control');
    }
    if (result.hasDefine) {
        score += 10;
        result.reasons.push('define() shared helpers');
    }
    if (result.hasGetChatMessages) {
        score += 10;
        result.reasons.push('getChatMessages() context scan');
    }
    if (result.hasExecute) {
        score += 5;
        result.reasons.push('execute() slash commands');
    }
    if (result.hasDecorators) {
        score += 5;
        result.reasons.push('Decorators detected');
    }
 ...
 */
export function extractEjsEntryNames(card) {
    const data = (card.data || card);
    const entries = data.character_book?.entries || [];
    ...
    entries.forEach((entry, i) => {
        ...
        if (comment)
            entryNameMap.set(comment, i);
        if (name && name !== comment)
            entryNameMap.set(name, i);
    });
 ...
 */
export function extractEjsKeywords(card) {
    const allTexts = collectAllTexts(card);
    const keywords = [];
    const seen = new Set();
    const addKeyword = (keyword, type, foundIn, context) => {
    ...
    for (const { text, source } of allTexts) {
        ...
        while ((dr = decoratorRegexPattern.exec(text)) !== null) {
            ...
            for (const part of parts) {
                ...
                if (trimmed)
                    addKeyword(trimmed, 'decorator_regex', source);
            }
        }
        ...
        for (const block of ejsBlocks) {
            ...
            while ((kv = objKvPattern.exec(block)) !== null) {
                ...
                if (hasCJKKey)
                    addKeyword(kv[1], 'alias', source, kv[0]);
                if (hasCJKVal)
                    addKeyword(kv[2], 'alias', source, kv[0]);
            }
        }
 ...
 */
export function extractEjsDecorators(text, fieldPath) {
    const decorators = [];
    ...
    for (const line of lines) {
        ...
        if (/^@@\w+/.test(trimmed)) {
            let type = 'other';
            if (trimmed.startsWith('@@render_after') || trimmed.startsWith('@@render_before'))
                type = 'render';
            else if (trimmed.startsWith('@@iframe'))
                type = 'iframe';
            else if (trimmed.startsWith('@@if') || trimmed.startsWith('@@else') || trimmed.startsWith('@@end'))
                type = 'if';
 ...
 */
export function extractAllDecorators(card) {
    const allTexts = collectAllTexts(card);
    const decorators = [];
    for (const { text, source } of allTexts) {
 ...
 */
export async function aiTranslateEjsEntries(entryNames, keywords, targetLang, proxy, signal, cardContext) {
    const allItems = [
...
Output ONLY the JSON object. No markdown, no explanation.`;
    ...
    try {
        ...
        if (err instanceof Error && (err.message === 'Cancelled' || signal?.aborted))
            throw err;
 ...
 */
export function validateEjsSync(fields, ejsEntryNameDict, ejsKeywordDict) {
    const report = {
        totalEntryNames: 0,
        matchedEntryNames: 0,
        missingEntryNames: [],
    ...
    for (const [original, translated] of entryEntries) {
        ...
        const getwiPattern = new RegExp('(?:getwi|getWorldInfo|activewi|activateWorldInfo)\\s*\\(\\s*(?:null|\'\'|""|[\\w.]+)\\s*,\\s*' + quoteClass + escapeRegex(translated) + quoteClass);
        const originalGetwiPattern = new RegExp('(?:getwi|getWorldInfo|activewi|activateWorldInfo)\\s*\\(\\s*(?:null|\'\'|""|[\\w.]+)\\s*,\\s*' + quoteClass + escapeRegex(original) + quoteClass);
 ...
 */
export function buildEjsPromptBlock(ejsEntryNameDict, ejsKeywordDict, ejsDecoratorPreserve) {
    let block = '';
